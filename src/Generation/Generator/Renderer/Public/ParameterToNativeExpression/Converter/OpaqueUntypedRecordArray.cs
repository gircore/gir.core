...
internal class OpaqueUntypedRecordArray : ToNativeParameterConverter
{
    public bool Supports(GirModel.AnyTypeReference anyTypeReference)
        => anyTypeReference.ReferencesArray<GirModel.Record>(out var record) && Model.Record.IsOpaqueUntyped(record);
    ...
    public void Initialize(ParameterToNativeData parameterData, IEnumerable<ParameterToNativeData> parameters)
    {
        switch (parameterData.Parameter)
        {
            ...
            case { Direction: GirModel.Direction.In }:
                ...
                break;
            default:
                throw new Exception($"{parameterData.Parameter}: This kind of opaque untyped array is not yet supported");
        }
    }
    ...
    private static void Ref(ParameterToNativeData parameter)
    {
        ...
        //TODO
        throw new Exception("Test missing for opaque untyped record array passed in via a ref");
    }
    ...
    private static void Span(ParameterToNativeData parameter, IEnumerable<ParameterToNativeData> allParameters)
    {
        ...
        switch (lengthParameter.Parameter.Direction)
        {
            case GirModel.Direction.In:
                ...
                break;
            default:
                throw new Exception("Unknown direction for length parameter in opaque untyped record array");
        }
    }
}
