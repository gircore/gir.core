    ...
    public void Fixup(GirModel.Class @class)
    {
        foreach (var method in @class.Methods)
        {
            var parameter = method.Parameters.FirstOrDefault(x => x is { CallerAllocates: true, Direction: Direction.Out }
                                                                  && x.AnyTypeReferenceOrVarArgs.TryPickT0(out var anyTypeTypeReference, out _)
                                                                  && anyTypeTypeReference.TryPickT1(out var arrayTypeReference, out _)
                                                                  && arrayTypeReference.Length is not null);

            if (parameter is null)
                continue;

            var lengthParameter = method.Parameters.ElementAt(parameter.AnyTypeReferenceOrVarArgs.AsT0.AsT1.Length!.Value);
